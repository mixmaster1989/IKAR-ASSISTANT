...
class InternetIntelligenceSystem:
    """Революционная система интернет-интеллекта"""
    ...
    def __init__(self, openrouter_api_key: str = None):
        ...
        # Источники новостей (более надежные)
        self.news_sources = [
    ...
    async def _search_duckduckgo(self, query: str, max_results: int = 6) -> List[SearchResult]:
        ...
        try:
            ...
            # Используем DuckDuckGo HTML поиск вместо API
                ...
                if response.status == 200:
                    ...
                    for result in soup.select('.result')[:max_results]:
                        try:
                            ...
                            logger.warning(f"Ошибка парсинга DuckDuckGo результата: {e}")
        ...
        try:
            ...
            for rss_url in self.news_sources[:3]:  # Ограничиваем количество источников
                try:
                    ...
                    logger.warning(f"Таймаут RSS {rss_url}")
                except Exception as e:
    ...
    async def extract_content(self, search_results: List[SearchResult]) -> List[SearchResult]:
        """Извлечение полного контента с веб-страниц"""
            ...
            try:
                ...
                async with session.get(result.url, timeout=10) as response:
                    if response.status == 200:
                        ...
                        if extracted:
                            result.content = extracted[:5000]  # Ограничиваем размер
                        else:
                            # Fallback на newspaper3k
                            try:
                                article = Article(result.url)
                                article.download()
                                article.parse()
                                result.content = article.text[:5000]
                            except:
    ...
    async def _generate_ai_summary(self, query: str, content: str) -> str:
        ...
        try:
            ...
            data = {
                ...
                "messages": [
                    {"role": "system", "content": "Ты эксперт по анализу и структурированию информации из интернета."},
            ...
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    if "choices" in result and len(result["choices"]) > 0:
                        ...
                        logger.warning("Неожиданный формат ответа от OpenRouter")
                        return self._simple_summary(content)
                else:
                    logger.error(f"Ошибка OpenRouter API: {response.status}")
                    return self._simple_summary(content)

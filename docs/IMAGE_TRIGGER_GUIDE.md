...
- **Cooldown**: отключён для основных триггеров
...
class GroupImageTrigger:
    def __init__(self):
        self.cooldown_sec = 0  # кулдаун отключен
        self.trigger_word = "ботнарисуй"
```

## 🖼️ **Автоматическая отправка KKT-картинок**
В ответах ассистента автоматически распознаются упоминания моделей кассовой техники и отправляются изображения из каталога `kktpictures/`.

Поддерживаемые соответствия (пример):
...
- Атол 91Ф → `kktpictures/91.png`
- Атол 30Ф → `kktpictures/30.jpeg`
- Атол 20Ф → `kktpictures/20.jpeg`
- Атол Sigma 10 → `kktpictures/sigma10.jpeg`
- Атол Sigma 7 → `kktpictures/sigma7.jpeg`
- Атол Sigma 8 → `kktpictures/atol-sigma-8.png`

Реализация: функция `_maybe_send_kkt_picture()` в `backend/api/telegram_core.py`.
